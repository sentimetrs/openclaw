...
describe("slack prepareSlackMessage inbound contract", () => {
  ...
  it("uses thread session key for top-level messages when replyToMode=all", async () => {
    const slackCtx = createSlackMonitorContext({
      cfg: {
        channels: { slack: { enabled: true, replyToMode: "all" } },
      } as OpenClawConfig,
    ...
    expect(prepared).toBeTruthy();
    // Session key should include :thread: suffix even for top-level messages
    expect(prepared!.ctxPayload.SessionKey).toContain(":thread:");
    expect(prepared!.ctxPayload.SessionKey).toContain("1.000");
  });

  it("does not use thread session key when replyToMode=off", async () => {
    const slackCtx = createSlackMonitorContext({
      cfg: {
        channels: { slack: { enabled: true } },
      } as OpenClawConfig,
    ...
    expect(prepared).toBeTruthy();
    // Session key should NOT include :thread: for replyToMode=off
